...
만약 뱀을 밟아서 뒤로 돌아갔을때 그 점이 처음 방문한 점이어도 나중에 그냥 주사위만 굴려서 다시한번 방문했을때의 주사위를 굴린 횟수보다 더 크지 않고, 그 반대의 경우에도 먼저 방문했을때의 경우가 최소가 된다는것을 알게되었다.
이를 통해 BFS가 최단경로를 보장한다는 것을 확신하게 되었다.
