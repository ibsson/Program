...
처음으로 스스로 DFS문제를 풀어내었다. 몇달 전까지만 해도 손도 못댔던 문제를 풀어내었다. 내가 성장하는게 느껴져서 기분이 좋았다.
...
그리고 DFS는 '효율성'이 중요하다는 것을 알게 되었다. 
재귀로 깊이 들어갈수록 메모리를 많이 차지해 이미 검사한 노드는 생략하는 등 효율적으로 만드는 것이 중요하다고 느꼈고, 스택으로도 DFS를 구현해 이 문제를 한번 더 풀어봐야겠다.
*/
