...
만약 뱀을 밟아서 뒤로 돌아갔을때 그 점이 처음 방문한 점이어도 나중에 그냥 주사위만 굴려서 다시한번 방문했을때의 주사위를 굴린 횟수보다 더 크지 않다는 것을 알게되었다.
앞으로 BFS문제를 풀때 최단경로를 보장해준다는 점을 전보다 더 잘 활용해서 쉽게 만들 수 있을 것 같다.
*/
