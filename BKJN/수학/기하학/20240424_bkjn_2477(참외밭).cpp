...
/*
처음에는 좌표를 이용한다는 생각을 못하고 그냥 (큰 사각형 - 작은 사각형) 이 생각만 가지고 문제를 일반화를 하려 했다. 하지만 쉽지 않았다. 그래서 좌표를 이용했다.
...
일반화 하는것이 코딩할때 굉장히 중요하다는 것을 느꼈으며, 또 일반화 하는 것이 가장 어렵다는 것과, 일반화만 된다면 코드는 금방 짠다는 것을 느꼈다.
